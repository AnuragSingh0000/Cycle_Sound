...
def visualize_spectrogram(audio_spect, epoch=0, sr=22050, hop_length=256, save_dir="saved_spectrograms"):
    ...
    for i in range(num_images):
        ...
        plt.colorbar()
        plt.tight_layout()
        save_path = os.path.join(save_dir, f"spectrogram_{epoch}_{i+1}.png")
        plt.savefig(save_path)
        plt.close()
